...
#define FILTERS_C_IMPLEMENTATION 1
    ...
    Similar, but not a one to one conversion of the C code above.Try to find in what way.__asm__ __volatile__(
                 "subq $0x18, %%rsp\n\t"
...
#define FILTERS_C_IMPLEMENTATION 1
    ...
    const size_t window_width =
        FILTERS_MEDIAN_WINDOW_SIZE % 2 == 0 ? FILTERS_MEDIAN_WINDOW_SIZE + 1 : FILTERS_MEDIAN_WINDOW_SIZE;
...
#error "Unsupported processor architecture"
        ...
        uint8_t median =
            window_size % 2 == 0 ? (uint8_t)((window[window_center - 1] + window[window_center]) * 0.5) : (uint8_t)window[window_center];
